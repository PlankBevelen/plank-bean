...
import type { PatternParameterMode, PatternSystemRecommendation } from '../types'
import { usePatternStore } from '../store'
import {
  BEAD_DISPLAY_SIZE,
  BEAD_PALETTE,
  buildSystemPatternRecommendationFromImage,
  drawPatternToCanvas,
...
export default function Home() {
  ...
  const [systemRecommendation, setSystemRecommendation] = useState<PatternSystemRecommendation | null>(null)
  ...
  useEffect(() => {
    ...
    async function buildRecommendation() {
      try {
        const nextRecommendation = await buildSystemPatternRecommendationFromImage(currentImageSrc)
        if (!cancelled) {
          setSystemRecommendation(nextRecommendation)
        }
      } catch (error) {
        if (!cancelled) {
          console.error('生成系统推荐设置失败', error)
          setSystemRecommendation(null)
        }
      }
    }
  ...
  const handleApplyRecommendation = useCallback(() => {
    ...
    applyParameterPatch({
      gridSize: systemRecommendation.gridSize,
      processingOptions: systemRecommendation.processingOptions,
    })
  }, [applyParameterPatch, systemRecommendation])
